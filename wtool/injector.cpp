...
HINSTANCE ManualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, DWORD dwProcessId , bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	if (reinterpret_cast<IMAGE_DOS_HEADER*>(pSrcData)->e_magic != 0x5A4D) { //"MZ"
		ILog("[ManualMapDll] 失败：DOS 头签名错误，e_magic=0x%04X（应为 0x5A4D 即 'MZ'）\n",
	...
	if (pOldFileHeader->Machine != CURRENT_ARCH) {
		ILog("[ManualMapDll] 失败：位数不匹配，DLL 的 Machine=0x%04X，当前程序架构=0x%04X\n",
	...
	if (!pTargetBase) {
		ILog("[ManualMapDll] 失败：在目标进程分配镜像内存失败，SizeOfImage=0x%lX，错误码=%lu\n",
			(DWORD)pOldOptHeader->SizeOfImage, GetLastError());
		return nullptr;
	}
	ILog("[ManualMapDll] 成功：在目标进程分配镜像内存，基址=0x%p，大小=0x%lX\n",
		(void*)pTargetBase, (DWORD)pOldOptHeader->SizeOfImage);
	...
	if (!WriteProcessMemory(hProc, pTargetBase, pSrcData, 0x1000, nullptr)) {
		ILog("[ManualMapDll] 失败：写入 PE 头失败，基址=0x%p，错误码=%lu\n",
			(void*)pTargetBase, GetLastError());
	...
	for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {
		if (pSectionHeader->SizeOfRawData) {
			if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, pSrcData + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr)) {
				ILog("[ManualMapDll] 失败：写入节区失败，节名=%.8s，RVA=0x%lX，大小=0x%lX，错误码=%lu\n",
					(const char*)pSectionHeader->Name, (DWORD)pSectionHeader->VirtualAddress,
					(DWORD)pSectionHeader->SizeOfRawData, GetLastError());
				VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
				return nullptr;
			}
		}
	...
	if (!MappingDataAlloc) {
		ILog("[ManualMapDll] 失败：分配映射参数内存失败，大小=0x%llX，错误码=%lu\n",
			(unsigned long long)sizeof(MANUAL_MAPPING_DATA), GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		return nullptr;
	}
	...
	if (!WriteProcessMemory(hProc, MappingDataAlloc, &data, sizeof(MANUAL_MAPPING_DATA), nullptr)) {
		ILog("[ManualMapDll] 失败：写入映射参数失败，地址=0x%p，错误码=%lu\n",
			(void*)MappingDataAlloc, GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
	...
	if (!pShellcode) {
		ILog("[ManualMapDll] 失败：分配 Shellcode 内存失败（0x1000），错误码=%lu\n", GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
		return nullptr;
	}
	...
	if (!WriteProcessMemory(hProc, pShellcode, Shellcode, 0x1000, nullptr)) {
		ILog("[ManualMapDll] 失败：写入 Shellcode 失败，地址=0x%p，错误码=%lu\n",
			(void*)pShellcode, GetLastError());
		VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
		VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
	...
	DWORD tid = FindGuiThreadId(dwProcessId);
	HANDLE hThread = nullptr;
	if (!KernelOpenThread(core, dwProcessId, tid, hThread) || hThread == nullptr) {
		ILog("[ManualMapDll] 失败：内核打开目标线程失败，pid=%lu，tid=%lu（tid=0 表示未找到 GUI 线程）\n",
			dwProcessId, tid);
	...
	*(ULONGLONG*)(stub + 42) = originalRIP;
	...
	ILog("[ManualMapDll] 成功：线程劫持已设置，pid=%lu，tid=%lu，现RIP=0x%llX\n",
		dwProcessId, tid, pRemoteStub);
	getchar();
	...
	while (!hCheck) {
		...
		if (exitcode != STILL_ACTIVE) {
			ILog("[ManualMapDll] 失败：目标进程已退出，pid=%lu，退出码=%lu\n", dwProcessId, exitcode);
			return nullptr;
		}
		...
		if (hCheck == (HINSTANCE)0x404040) {
			ILog("[ManualMapDll] 失败：Shellcode 返回错误标记 0x404040（pData 为空 / 未正常执行），pid=%lu\n", dwProcessId);
			VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
			VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE);
			VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
			return nullptr;
			...
			ILog("[ManualMapDll] 警告：RtlAddFunctionTable 失败（0x505050），SEH 异常支持未启用\n");
		}
	...
	if (emptyBuffer == nullptr) {
		ILog("[ManualMapDll] 失败：本地申请 20MB 清零缓冲区失败（malloc 返回空）\n");
		return nullptr;
	}
	...
	ILog("[ManualMapDll] 成功：DLL 已映射，pid=%lu，目标进程内基址=0x%p\n", dwProcessId, (void*)hCheck);
	return hCheck;
}
...
HINSTANCE Mapdll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, DWORD Pid)
{
	return ManualMapDll(hProc, pSrcData, FileSize, Pid);
}


bool RemoteCall(HANDLE hProc, intptr_t  add, DWORD piid) {
	...
	BYTE stubTemplate[] = {
		...
		// 调用目标地址
		0x48, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // mov  rax, <target_address>
		0xFF, 0xD0,                                                  // call rax
	...
	DWORD tid = FindGuiThreadId(piid);
	HANDLE hThread = nullptr;
	if (!KernelOpenThread(core, piid, tid, hThread) || hThread == nullptr) {
		VirtualFreeEx(hProc, pRemoteStub, 0, MEM_RELEASE);
		ILog("[RemoteCall] 失败：内核打开目标线程失败，pid=%lu，tid=%lu（tid=0 表示未找到 GUI 线程）\n",
			piid, tid);
		return false;
	}
	GrantHandleAccess(core, (uint64_t)hThread);
	...
	BYTE stub[sizeof(stubTemplate)];
	memcpy(stub, stubTemplate, sizeof(stubTemplate));
	*(ULONGLONG*)(stub + 11) = (ULONGLONG)add;
	*(ULONGLONG*)(stub + 32) = originalRIP;
	WriteProcessMemory(hProc, pRemoteStub, stub, sizeof(stub), NULL);
	// 修改rip
	ctx.Rip = (ULONGLONG)pRemoteStub;
	SetThreadContext(hThread, &ctx);
